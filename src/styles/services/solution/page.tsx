...
import { 
    FaUniversity, FaFlask, FaRegHospital, FaClinicMedical, FaAmbulance, FaMicroscope, FaBrain,
    FaUserTie, FaConnectdevelop, FaGlobe, FaLaptopCode, FaLeaf 
} from 'react-icons/fa';
...
const solutionsData: SolutionsData = {
    ...
    value: {
        ...
        points: [
           { icon: FaUserTie, title: "Owner’s Perspective", text: "We solve problems the way promoters and trustees need them solved." },
           { icon: FaConnectdevelop, title: "Integrated Approach", text: "Combining finance, design, EPC, and operations under one platform." },
           { icon: FaGlobe, title: "Global Compliance", text: "Facilities aligned with NABH, JCI, WHO, LEED, WELL, and ESG standards." },
           { icon: FaLaptopCode, title: "Tech & Digital Innovation", text: "IoT-enabled hospitals, BIM-integrated designs, AI-driven facility management." },
           { icon: BsFillGrid3X3GapFill, title: "Future-Ready & Modular", text: "Adaptable to new specialties, pandemics, or evolving healthcare needs." },
           { icon: FaLeaf, title: "Sustainability", text: "Green buildings, energy-efficient MEPF systems, and reduced carbon footprint." }
        ]
    }
};
    ...
    <ul className="details-list">
        {items.map((item, index) => {
            if (typeof item === 'string') {
                return <li key={index}><BsCheckCircleFill className="list-icon" /><span>{item}</span></li>;
            }
            return (
                <li key={index}>
                    ...
                    <div>
                        ...
                        {item.sub.map((subItem, subIndex) => (
                            <div key={subIndex} className="sub-item">{subItem}</div>
                        ))}
                    </div>
                </li>
            );
                    ...
                    <Col lg={3} className="d-none d-lg-block">
                        <Nav className="flex-column sticky-top sidenav">
                            {solutionsData.spectrum.map(category => (
                                <Nav.Link key={category.id} href={`#${category.id}`} onClick={(e) => handleNavLinkClick(e, category.id)} className={activeLink === category.id ? 'active' : ''}>
                                    {category.title}
                                </Nav.Link>
                            ))}
                             <Nav.Link href="#value" onClick={(e) => handleNavLinkClick(e, 'value')} className={activeLink === 'value' ? 'active' : ''}>How We Add Value</Nav.Link>
                        </Nav>
                    </Col>
                                ...
                                {solutionsData.spectrum.map((category, index) => (
                                    <Row as="section" key={category.id} id={category.id} className="mb-5 interactive-section align-items-center">
                                         <Col md={5} className={index % 2 === 0 ? 'order-md-1' : 'order-md-2'}>
                                            <img src={`https://placehold.co/600x400/f0fdfa/3b9c9d?text=${category.title.replace('&', '%26')}`} alt={category.title} className="img-fluid" />
                                         </Col>
                                         <Col md={7} className={index % 2 === 0 ? 'order-md-2' : 'order-md-1'}>
